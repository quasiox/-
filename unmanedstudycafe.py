...
# ═══════════════════════════════════════════
def _ticket_priority(ttype: int) -> int:
    """낮을수록 먼저 소진 (사용자에게 유리한 순).

    시간권(2) > 정기권(1) > 기간권(4) > 종일권(3)

    근거:
      시간권 — 자리비움 절반 차감 혜택이 있으므로 먼저 소진해야 혜택을 충분히 누림.
               중도퇴장 불가이므로 활성화 후 다른 이용권과 교체 불가.
      정기권 — 분 단위 차감, 자리비움 없음.
      기간권 — 일 단위 차감.
      종일권 — 활성화된 날 자정까지만 유효. 대기 중에는 소모되지 않으므로
               나중에 활성화해도 당일 자정까지 쓸 수 있어 가장 여유 있음.

    같은 종류끼리는 userticket_id 오름차순(구매 순서 빠른 것)으로 결정.
    """
    return {2: 0, 1: 1, 4: 2, 3: 3}.get(ttype, 99)


class UserTicketEntry:
    """UserTicketRelation.txt 한 행.

    형식: userticket_id.user_id.ticket_id.is_reserved
      userticket_id : 보유 기록 고유번호 (PK, 자동 증가)
      user_id       : UserRelation 참조
      ticket_id     : TicketRelation 참조 (이용권 상품 ID)
      is_reserved   : 예약 파트에서 이 이용권을 예약에 묶었는지 여부
                      0 = 대기 중 (소진 순서에 포함)
                      1 = 예약에 묶임 (소진 순서에서 제외, 예약 파트가 설정/해제)

    remain을 저장하지 않는 이유:
      대기 이용권은 아직 활성화 전이므로 항상 최대값.
      TicketRelation.duration으로 계산 가능하여 중복 저장 불필요.
    """
    def __init__(self, userticket_id: int, user_id: str,
                 ticket_id: int, is_reserved: bool = False):
...
class StudyCafe:
    def __init__(self):
        ...
        self.userticket_entries: list[UserTicketEntry] = []  # UserTicketRelation
    ...
    def verify_integrity(self):
        """릴레이션 무결성 검사 (유저 > 티켓 > 좌석 > 세션 > 보유이용권 순)"""
    ...
    def _verify_userticket_relation(self):
        ...
        - user_id: UserRelation 참조 무결성
        - ticket_id: TicketRelation 참조 무결성
        - 현재 이용권 없는 유저의 항목 없음
        - 같은 유저 내 is_reserved=True 항목 2개 이상 불가
        """
        user_map   = {u.id: u for u in self.users}
        ticket_ids = {t.id for t in self.tickets}
        seen_ids   = set()
    ...
    def _sorted_queue(self, user: User) -> list:
        """해당 유저의 대기 이용권을 우선순위 순으로 정렬한 복사본 반환.

        정렬 기준:
          1순위: _ticket_priority 오름차순 (종류별 긴급도)
          2순위: userticket_id 오름차순 (같은 종류 내 구매 순서)
        """
        def key(e):
    ...
    def _enqueue_ticket(self, user: User, ticket: Ticket):
        """새 이용권 구매 시 우선순위에 맞게 현재 이용권/대기열에 삽입.

        동작:
          1. 현재 이용권이 없으면 바로 활성화.
          2. 새 이용권이 현재 이용권보다 우선순위 높으면:
             현재 이용권을 대기열에 내리고 새 이용권을 현재로 활성화.
          3. 우선순위 같거나 낮으면 대기열에 추가.
        """
        if not user.has_ticket():
        ...
        if new_prio < cur_prio:
            # 새 이용권이 더 우선 → 현재 이용권 대기열에 내리고 스왑
    ...
    def _activate_next_ticket(self, user: User) -> bool:
        """현재 이용권 만료 후 대기열에서 다음 이용권 자동 활성화.

        is_reserved=True 항목은 건너뜀 (예약 파트에서 해당 이용권 사용 예약 중).

        반환값:
          True  — 활성화 성공 (좌석 유지)
          False — 활성화 가능한 대기 이용권 없음 (퇴장 처리 필요)
        """
        ...
        # 입장 중이면 새 이용권으로 새 세션 시작 (차감이 이어지도록)
        seat = self._find_seat_by_user(user.id)
        if seat:
            user.start_time = now
            self.sessions.append(
                Session(user.id, user.ticket_id, seat.id, now))
        else:
            user.start_time = None
    ...
    def _show_cmds_logged_in(self):
        ...
        print("move 자리이동        | 없음               | 다른 빈 좌석으로 이동")
    ...
    def cmd_help(self, args: list[str]):
        ...
        help_texts = {
            ...
            "move": "현재 좌석에서 다른 빈 좌석으로 이동합니다.\n동의어: move 자리이동\n인자: 없음\n동작: 입장 중일 때만 가능. 시간권 자리비움 중에도 이용 가능 (상태 유지)",
    ...
    def cmd_login(self, args: list[str]):
        ...
        if expired:
            ...
            print(f"..! 안내: 대기 이용권이 {cnt}개 있습니다. 'myinfo'로 확인하세요.")
    ...
    def cmd_buy(self, args: list[str]):
        """이용권 구매. 이미 이용권이 있어도 추가 구매 가능 (중복 이용권).

        소진 우선순위 (사용자에게 유리한 순):
          시간권 > 정기권 > 기간권 > 종일권
        같은 종류끼리는 먼저 구매한 것(userticket_id 오름차순) 먼저 소진.
        is_reserved=True 항목은 예약에 묶여 소진 순서에서 제외.
        """
        now = self.get_now()
        if args:
            print(".!! 오류: 인자가 없어야 합니다.")
    ...
    def cmd_resume(self, args: list[str]):
        ...
        # 자리비움 구간까지의 차감을 remain에 확정 후, start_time을 now로 갱신.
        # 이렇게 하면 이후 _calc_deduction은 now 이후 구간만 다시 계산한다.
        deduction = self._calc_deduction(user, ticket, now)
        user.remain = max(0, user.remain - deduction)

        # 자리비움 해제: away_start 초기화, start_time을 현재 시각으로 갱신
        user.away_start = None
        user.start_time = now

        self._save_users()
        eff = self._calc_effective_remain(user, now)
        print("... 자리비움을 해제합니다. 이용 시간이 정상 속도로 차감됩니다.")
        print(f"    잔여 시간: {fmt_minutes(eff)}")

    def cmd_move(self, args: list[str]):
        """현재 좌석에서 다른 빈 좌석으로 이동합니다.

        - 입장 중일 때만 사용 가능 (모든 이용권 유형)
        - 시간권 자리비움(pause) 중에도 이동 가능 → 자리비움 상태 그대로 유지
        - 이동 전까지 사용된 시간을 remain에 확정한 뒤, start_time/away_start를
          이동 시각으로 갱신해 이중 차감을 방지
        - 기존 세션을 닫고 새 좌석으로 새 세션을 시작
        - 종일권/기간권은 시간 차감 없이 좌석·세션만 교체
        """
        if args:
            print(".!! 오류: 인자가 없어야 합니다.")
            return
        ...
        print("=== 자리이동 ===")
        print(f"    현재 좌석: {old_seat.id}번")
        if user.is_away():
            print("    ※ 자리비움 중입니다. 이동 후에도 자리비움 상태가 유지됩니다.")
        print()
        self._print_seats()

        while True:
            seat_input = safe_input("선택: 이동할 좌석 번호 > ")
            if seat_input is None:
                self._handle_eof()
                return
            ...
            if new_seat_num < 1 or new_seat_num > TOTAL_SEATS:
                print(f".!! 오류: 유효하지 않은 좌석 번호입니다. (1~{TOTAL_SEATS})")
                continue
            if new_seat_num == old_seat.id:
                print(".!! 오류: 현재 좌석과 동일한 번호입니다. 다른 좌석을 선택하세요.")
                continue
            new_seat = self.seats[new_seat_num - 1]
            if not new_seat.is_empty():
                print(f".!! 오류: {new_seat_num}번 좌석은 이미 사용 중입니다.")
                continue
            break

        # 시간권·정기권: 이동 전 구간 차감 확정 후 기준 시각 갱신
        # (이렇게 해야 이동 후 _calc_deduction이 이동 시각 이후만 계산함)
        if ticket and ticket.type in (1, 2):
            deduction = self._calc_deduction(user, ticket, now)
            user.remain = max(0, user.remain - deduction)

            # 잔여 시간 소진 → 이동 불가, 퇴장 처리
            if user.remain <= 0:
                for s in reversed(self.sessions):
                    if s.user_id == user.id and s.exit_time is None and not s.is_shutdown_record():
                        s.exit_time = now
                        s.usage_min = math.floor(
                            (now - s.enter_time).total_seconds() / 60)
                        break
                old_seat.user_id = ""
                user.away_start = None
                if not self._activate_next_ticket(user):
                    ...
                    user.away_start = None
                self.save_all()
                print(".!! 안내: 이용권 잔여 시간이 소진되어 자리이동 대신 퇴장 처리되었습니다.")
                return

            # 시간권: start_time을 이동 시각으로 갱신
            # 자리비움 중이면 away_start도 이동 시각으로 갱신 (자리비움 상태 유지)
            if ticket.type == 2:
                user.start_time = now
                if user.is_away():
                    user.away_start = now
            # 정기권: start_time을 이동 시각으로 갱신
            elif ticket.type == 1:
                user.start_time = now
        ...
        # 새 세션 시작 (이동 시각부터)
        self.sessions.append(
            Session(user.id, user.ticket_id, new_seat.id, now))

        self.save_all()

        away_note = " (자리비움 상태 유지)" if user.is_away() else ""
        print(f"... {old_seat.id}번 좌석에서 {new_seat.id}번 좌석으로 이동하였습니다.{away_note}")
        if ticket and ticket.type in (1, 2):
            eff = self._calc_effective_remain(user, now)
            print(f"    잔여 시간: {fmt_minutes(eff)} ({ticket.type_name()})")
